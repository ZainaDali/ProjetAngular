...
@Component({
  ...
  template: `
    <div class="max-w-2xl mx-auto mt-6 p-6 bg-white shadow rounded">
      ...
      <ul class="space-y-3">
        @for (s of symptomesPatient(); track s.id) {
          <li class="border p-3 rounded" [appHighlight]="s.gravite">
                      ...
                      <div class="flex items-center gap-2">
                        ...
                        @if (noteEnEdition?.id === n.id) {
                          <button class="text-green-600 text-sm"
                                  (click)="sauverNote(s.id, n.id)">
                            Sauver
                          </button>
                        }

                        <button class="text-red-600 text-sm" (click)="supprimerNote(s.id, n.id)">
                          Supprimer
                        </button>
                      </div>
                    </li>
